...
#ifndef SWIFTTRIMMING_H
#define SWIFTTRIMMING_H
...
class SwiftTrimmingSink;
// class LogSimInterface;

class SentPacketSwift {
  public:
    SentPacketSwift(simtime_picosec t, uint64_t s, bool a, bool n, bool to)
            : timer{t}, seqno{s}, acked{a}, nacked{n}, timedOut{to} {}
    SentPacketSwift(const SentPacketSwift &sp)
...
class SwiftTrimmingSrc : public PacketSink,
  ...
  public:
    SwiftTrimmingSrc(SwiftTrimmingLogger *logger, TrafficLogger *pktLogger,
                     EventList &eventList, uint64_t rtt, uint64_t bdp,
                     uint64_t queueDrainTime, int hops);
    // SwiftTrimmingSrc(SwiftTrimmingLogger *logger, TrafficLogger* pktLogger,
    // EventList& eventList, uint64_t rtt=timeFromUs(5.25), uint64_t bdp=63000);
    ~SwiftTrimmingSrc();
    ...
    virtual void connect(Route *routeout, Route *routeback,
                         SwiftTrimmingSink &sink, simtime_picosec startTime);
    void startflow();
  ...
  private:
    ...
    int rx_count = 0;
    int achieved_bdp = 0;
    SwiftTrimmingLogger *_logger;
    SwiftTrimmingSink *_sink;
    ...
    vector<SentPacketSwift> _sent_packets;
...
class SwiftTrimmingSink : public PacketSink, public DataReceiver {
  ...
  private:
    ...
    int ack_count_idx = 0;
    string _nodename;
    list<SwiftTrimmingAck::seq_t> _received; // list of packets received OOO
    uint16_t _crt_path;
    const Route *_route;
    ...
    vector<const Route *> _original_paths; // paths in original permutation
                                           // order
    SwiftTrimmingSrc *_src;

    void send_ack(simtime_picosec ts, bool marked,
                  SwiftTrimmingAck::seq_t seqno, SwiftTrimmingAck::seq_t ackno,
                  const Route *rt, const Route *inRoute, int path_id);
    void send_nack(simtime_picosec ts, bool marked,
                   SwiftTrimmingAck::seq_t seqno, SwiftTrimmingAck::seq_t ackno,
                   const Route *rt, int);
    bool already_received(SwiftTrimmingPacket &pkt);
};

class SwiftTrimmingRtxTimerScanner : public EventSource {
  public:
    SwiftTrimmingRtxTimerScanner(simtime_picosec scanPeriod,
                                 EventList &eventlist);
    void doNextEvent();
    void registerSwiftTrimming(SwiftTrimmingSrc &uecsrc);

  private:
    simtime_picosec _scanPeriod;
    simtime_picosec _lastScan;
    typedef list<SwiftTrimmingSrc *> swifts_t;
    swifts_t _uecs;
};
