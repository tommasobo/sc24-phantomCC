...
#define RECORD_PATH_LENS // used for debugging which paths lengths packets were
...
class NdpSrc : public PacketSink, public EventSource, public TriggerTarget {
  ...
  public:
    ...
    static uint32_t _global_rto_count; // keep track of the total number of
...
#define MAX_PATH_LEN 20u
  ...
  private:
    ...
    inline uint32_t flow_id() const { return _src->flow_id(); };
...
class NdpPullPacer : public EventSource {
  public:
    ...
    // void set_preferred_flow(int id) { _preferred_flow = id;cout <<
    // "Preferring flow "<< id << endl;};
...
#define FAIR_PULL_QUEUE
    ...
    NdpPull::seq_t _pacer_no; // pull sequence number, shared by all connections
                              // on this pacer
