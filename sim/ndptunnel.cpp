...
void NdpTunnelSrc::processAck(const NdpAck &ack) {
    ...
    */
    ...
    if (_logger)
        _logger->logNdp(*this, NdpTunnelLogger::NDP_RCV);
...
const Route *NdpTunnelSrc::choose_route() {
    ...
    case SINGLE_PATH:
        abort(); // not sure if this can ever happen - if it can, remove this
                 // line
        return _route;
    case PULL_BASED: // added this to shut to compiler warnings - add real code
                     // if we ever hit this abort
        abort();
    case NOT_SET: {
        abort();
        break;
...
int NdpTunnelSrc::send_packet(NdpPull::seq_t pacer_no) {
    ...
    if (!_rtx_queue.empty()) {
        ...
        // there are no packets in the RTX queue, so we'll send a new one from
        // the EQDS queue.
        Packet *pkt = _queue.back();
        _queue.pop_back();

        _qs -= pkt->size();
        // cout << "Sending packet into tunnel, qs " << _qs << " packet size "
        // << pkt->size() << " flightsize" << _flight_size << endl;
...
void NdpTunnelSrc::update_rtx_time() {
...
void NdpTunnelSrc::process_cumulative_ack(NdpTunnelPacket::seq_t cum_ackno) {
...
void NdpTunnelSrc::retransmit_packet() {
    ...
    /*    NdpTunnelPacket* p;
      ...
      timeAsUs(_rto) << "us now " << timeAsUs(eventlist().now()) << "us\n";
...
void NdpTunnelSrc::doNextEvent() {
    if (_rtx_timeout_pending) {
        ...
        if (_logger)
            _logger->logNdp(*this, NdpTunnelLogger::NDP_TIMEOUT);
...
NdpTunnelSink::NdpTunnelSink(EventList &event, linkspeed_bps linkspeed,
    ...
    //_pacer = new NdpTunnelPullPacer(event,
...
/* Use this constructor when there are multiple flows to one receiver
   ...
   NdpTunnelPullPacer */
NdpTunnelSink::NdpTunnelSink(NdpTunnelPullPacer *pacer)
        : DataReceiver("ndp_sink"), _cumulative_ack(0), _total_received(0) {
    _src = 0;
    _pacer = pacer;
    _nodename = "ndpsink";
...
/* sets the set of paths to be used when sending from this NdpTunnelSink back to
 * the NdpTunnelSrc */
...
void NdpTunnelSink::receivePacket(Packet &pkt) {
    ...
    if (pkt.header_only()) {
        ...
        // not releasing this packet since it will be released by the sender
    ...
    if (seqno == _cumulative_ack + 1) { // it's the next expected seq no
        // cout << "Normal Delivery new PKT " << seqno << " ACK " <<
        // _cumulative_ack << " flow " << p->flow().flow_id() << endl;
...
NdpTunnelPullPacer::NdpTunnelPullPacer(EventList &event, char *filename)
    ...
    if (!_pull_spacing_cdf) {
        ...
        for (int i = 0; i < _pull_spacing_cdf_count; i++) {
            if (fscanf(f, "%d %lf\n", &t, &_pull_spacing_cdf[i]) != 2) {
                cerr << "Parse error reading CDF value\n";
                exit(1);
            }
            // assert(t==i);
