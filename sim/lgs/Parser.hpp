...
class Graph {
  ...
  public:
    ...
    void write_as_dot() {
        /**
                Produces a dot representation of the graph. This is usefull for
           debugging purposes.
        */
        ...
        for (std::vector<Node *>::iterator it = allNodes.begin(); it != allNodes.end(); it++) {
            ...
            if ((**it).Type == OPTYPE_SEND)
                strcpy(typestr, "Send");
            else if ((**it).Type == OPTYPE_RECV)
                strcpy(typestr, "Recv");
            else if ((**it).Type == OPTYPE_CALC)
                strcpy(typestr, "Calc");
            else
                strcpy(typestr, "Unkn");
            fprintf(fd,
    ...
    /*
            void serialize(FILE* fd, uint32_t rank, uint32_t num_ranks) {
                    ...
                    if (rank == 0) {
                            ...
                            fseek(fd, sizeof(uint64_t)*2*num_ranks +
       sizeof(uint32_t), SEEK_SET);
                    }
                    else {
                            // jump to the end of the file
                            fseek(fd, sizeof(uint32_t) +
       (2*(rank-1)+1)*sizeof(uint64_t), SEEK_SET); uint64_t eos; fread(&eos,
       sizeof(uint64_t), 1, fd); fseek(fd, eos, SEEK_SET);
                    }
                    ...
                    long pos_in_appendix = start +
                                           sizeof(uint32_t)*2 + // for Nodecount
       and Independent Actions count sizeof(uint32_t)*RootNodes.size() + // for
       indp. actions offsets (sizeof(char) + sizeof(uint8_t)*2 +
       sizeof(uint32_t)*7 + sizeof(uint64_t)) * allNodes.size(); // for actual
       nodeinfo
                    ...
                    fwrite(&buf32, sizeof(uint32_t), 1, fd); // number of
       independent actions for (std::vector<Node*>::iterator it =
       RootNodes.begin(); it != RootNodes.end(); it++) { buf32 = (**it).offset;
                            fwrite(&buf32, sizeof(uint32_t), 1, fd); // offset
       of independent action
                    ...
                    Node n;
                    int cnt = 0;
                    for (std::vector<Node*>::iterator it = allNodes.begin(); it
       != allNodes.end(); it++) {

                            cnt++;

                            // write the actual operation info - this is a bit
       complicated thanks to padding that the compiler might do... fwrite(
       &((**it).DependenciesCnt), sizeof(uint32_t), 1, fd ); fwrite(
       &((**it).Type), sizeof(char), 1, fd ); fwrite( &((**it).Peer),
       sizeof(uint32_t), 1, fd ); fwrite( &((**it).Size), sizeof(uint64_t), 1,
       fd ); fwrite( &((**it).Tag), sizeof(uint32_t), 1, fd ); fwrite(
       &((**it).Proc), sizeof(uint8_t), 1, fd ); fwrite( &((**it).Nic),
       sizeof(uint8_t), 1, fd );

                            // handling dependencies
                            buf32 = (**it).DependOnMe.size();
                            fwrite( &buf32, sizeof(uint32_t), 1, fd ); // number
       of ops depending on this node

                            if ((**it).DependOnMe.size() > 0) {
                                    fwrite( &num_in_appendix, sizeof(uint32_t),
       1, fd ); // start of dependant offsets in appendix
                            }
                            else {
                                    uint32_t b = 999;
                                    fwrite( &b, sizeof(uint32_t), 1, fd ); //
       undefined
                            }
       ...
       (**it).DependOnMe.begin(); dit != (**it).DependOnMe.end(); dit++) {
                                    fwrite( &((**dit).offset), sizeof(uint32_t),
       1, fd );
                            }
                            // adjust number of elements and position in
       appendix num_in_appendix += (**it).DependOnMe.size(); pos_in_appendix =
       ftell(fd);
                            // jump back to the node info (we're in appendix
       right now) fseek(fd, pos, SEEK_SET);

                            // handling start-dependencies
                            buf32 = (**it).StartDependOnMe.size();
                            fwrite( &buf32, sizeof(uint32_t), 1, fd ); // number
       of ops start-depending on this node if ((**it).StartDependOnMe.size() >
       0) { fwrite( &num_in_appendix, sizeof(uint32_t), 1, fd ); // start of
       start-dependant offsets in appendix
                            }
                            else {
                                    uint32_t b = -1;
                                    fwrite( &b, sizeof(uint32_t), 1, fd ); //
       undefinedx
                            ...
                            pos =  ftell(fd);
                            // write offsets of start-depending ops into
       appendix fseek(fd, pos_in_appendix, SEEK_SET); for
       (std::vector<Node*>::iterator dit = (**it).StartDependOnMe.begin(); dit
       != (**it).StartDependOnMe.end(); dit++) { fwrite( &((**dit).offset),
       sizeof(uint32_t), 1, fd );
                            }
                            // adjust number of elements and position in
       appendix num_in_appendix += (**it).StartDependOnMe.size();
       ...
       right now) fseek(fd, pos, SEEK_SET);
                    ...
                    // write jumptable info: the schedule started at start and
       ended at pos_in_appendix fseek(fd, 0, SEEK_SET);

                    // the number of ranks in this file
                    buf32 = num_ranks;
                    fwrite(&buf32, sizeof(uint32_t), 1, fd);
                    fseek(fd, sizeof(uint64_t)*2*rank + sizeof(uint32_t),
       SEEK_SET); uint64_t buf64 = start; fwrite(&buf64, sizeof(uint64_t), 1,
       fd); buf64 = pos_in_appendix; fwrite(&buf64, sizeof(uint64_t), 1, fd);
            }
    ...
    */
    ...
    void serialize_mmap(int fd, uint32_t rank, uint32_t num_ranks, uint8_t max_cpu, uint8_t max_nic) {
        ...
        *((uint32_t *)pos) = (uint32_t)allNodes.size();
        pos += sizeof(uint32_t); // number of nodes in the schedule
        *((uint32_t *)pos) = (uint32_t)RootNodes.size();
        ...
        for (std::vector<Node *>::iterator it = allNodes.begin(); it != allNodes.end(); it++) {
            ...
            *((char *)pos) = (**it).Type;
            pos += sizeof(char); // type of this action
            *((uint32_t *)pos) = (**it).Peer;
            pos += sizeof(uint32_t); // peer of this action
            *((uint64_t *)pos) = (**it).Size;
            pos += sizeof(uint64_t); // size of the data transfer / length of
                                     // the local calculation
            *((uint32_t *)pos) = (**it).Tag;
            pos += sizeof(uint32_t); // tag of the send/recv operation
            *((uint8_t *)pos) = (**it).Proc;
            pos += sizeof(uint8_t); // processor used for this action
            *((uint8_t *)pos) = (**it).Nic;
            pos += sizeof(uint8_t); // network interface used for this action
            *((uint32_t *)pos) = (**it).DependOnMe.size();
            ...
            *((uint32_t *)pos) = (**it).StartDependOnMe.size();
            pos += sizeof(uint32_t); // number of actions that depend on this
                                     // actions start
            *((uint32_t *)pos) = num_in_appendix;
            pos += sizeof(uint32_t); // start index of start-dependent actions
                                     // (in appendix)
...
class SerializedGraph {
  ...
  private:
    char *mapping_start; // argh. this should be void* but c++ is anal on
                         // void*-arithmetic
  ...
  public:
    ...
    void write_as_dot(std::string filename) {
        /**
                Produces a dot representation of the graph. This is usefull for
           debugging purposes.
        */
    ...
    void GetExecutableNodes(nodelist_t *ret_ptr) {
        ...
        for (uint32_t cnt = 0; cnt < executableNodes.size(); cnt++) {
            ...
            if (executableNodes[cnt].Type == OPTYPE_SEND)
                gp.type = OP_SEND;
            else if (executableNodes[cnt].Type == OPTYPE_RECV)
                gp.type = OP_RECV;
            else if (executableNodes[cnt].Type == OPTYPE_CALC)
                gp.type = OP_LOCOP;
...
class Parser {
  ...
  public:
    ...
    Parser(std::string filename, bool save_mem) {
        ...
        size_t result = fread(&magic_cookie, sizeof(uint64_t), 1, schedules_fd);
        if (result == 0) {
            exit(0);
        }

        if (magic_cookie == MAGIC_COOKIE_INVALID) {
            fprintf(stderr, "This is serialized goal schedule was invalidated "
                            "by a prior simulation run!\n");
            exit(EXIT_FAILURE);
        }
        if (magic_cookie != MAGIC_COOKIE) {
            fprintf(stderr, "This is not a serialized goal schedule - the "
                            "magic cookie is missing\n");
            exit(EXIT_FAILURE);
        }
        ...
        result = fread(&max_cpu, sizeof(uint8_t), 1, schedules_fd);
        if (result == 0) {
            exit(0);
        }
        result = fread(&max_nic, sizeof(uint8_t), 1, schedules_fd);
        if (result == 0) {
            exit(0);
        }

        if (save_mem == true) {
            // mmap can fail with map_private and prot_write on machines where
            // the virtual mem is smaller than the mapped region - so we fall
        ...
        if (mapping_start == MAP_FAILED) {
            fprintf(stderr, "mmap does not work on your system! Try to use the "
                            "--save-mem option.\n");
            exit(EXIT_FAILURE);
        }
    ...
    uint8_t GetNumCPU() { return max_cpu + 1; }

    uint8_t GetNumNIC() { return max_nic + 1; }
