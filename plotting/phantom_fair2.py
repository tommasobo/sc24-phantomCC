...
# Read the file
file_path = 'imp_values/send0.txt'
...
# Read the file
file_path = 'imp_values/send2.txt'
df = pd.read_csv(file_path, header=None, usecols=[0,1], names=['X', 'Y'])
print(df['X'])
df['Y'] = (df['Y'] * 2)
df['X'] = (df['X'] / 1000 / 1000)
sns.set(style="whitegrid")
plt.plot(df.iloc[::10, 0], df.iloc[::10, 1], linewidth=3.5,alpha=0.7)
file_path = 'imp_values/send1.txt'
df = pd.read_csv(file_path, header=None, usecols=[0,1], names=['X', 'Y'])
print(df['X'])
df['Y'] = (df['Y'] * 2)
df['X'] = (df['X'] / 1000 / 1000)
...
plt.axhline(y=100/12, color='k', linestyle='--', linewidth=2, alpha=0.5)
plt.axhline(y=100/8, color='k', linestyle='--', linewidth=2, alpha=0.5)
plt.axhline(y=99/4, color='k', linestyle='--', linewidth=2, alpha=0.5)
