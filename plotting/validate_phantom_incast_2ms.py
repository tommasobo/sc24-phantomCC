...
def updateGoalList(incast_am):
    ...
    goal_file = []
    goal_file.append("test{}_2ms_smal_l.bin".format(incast_am))
    goal_file.append("test{}_2ms_large_l.bin".format(incast_am))
    id_name = []
    percentage_trimmed = []
    runtime_to_ideal = []
    completion_times_raw = []
...
def create_plot(x_labels, y_values, type_plot, min_plot, max_plot, incast_am):
    ...
    if (incast_am == 16):
        plt.title('Comparing Different Configurartions ({} Senders)'.format(32),fontsize=17)
    else:
    ...
    plt.tight_layout()
    if (type_plot == "completion"):
        plt.savefig("2ms/out{}_completion.png".format(incast_am), bbox_inches='tight')
        plt.savefig("2ms/out{}_completion.pdf".format(incast_am), bbox_inches='tight')
    elif (type_plot == "trimming"):
        plt.savefig("2ms/out{}_trimming.png".format(incast_am), bbox_inches='tight')
        plt.savefig("2ms/out{}_trimming.pdf".format(incast_am), bbox_inches='tight')
    else:
        plt.savefig("2ms/out{}_trimming_after.png".format(incast_am), bbox_inches='tight')
        plt.savefig("2ms/out{}_trimming_after.pdf".format(incast_am), bbox_inches='tight')
    ...
    for idx_goal, goal_name in enumerate(goal_file):
        for idx_slowdown, slowdown_percentage in enumerate(phantom_slowdown):
            for idx_pq_type, pq_type in enumerate(phantom_type):
                ...
                ideal_completion_time = ((tot_mb_sent + tot_mb_sent*0.035) * incast_flow / 10 / 1000) + 2200
                ...
                number_trimmed_after.append(getNumTrimmedAfter(out_file, 15000))
                ...
                print(getNumTrimmedAfter(out_file, 15000))
                print()
